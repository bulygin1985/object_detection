...
class AdvancedTensorBoardCallback:
    def __init__(
        ...
        all_features = {'metrics', 'histograms', 'images', 'gradients', 'weights', 'embeddings'}
        self.enabled_features = set(enabled_features) if enabled_features else all_features
    ...
    def on_training_start(self, model: torch.nn.Module, optimizer: torch.optim.Optimizer):
    ...
    def _log_metrics(self, metrics: Dict[str, float], epoch: int):
        ...
        for name, value in metrics.items():
            ...
            if len(self.metrics_history[name]) > 1:
                avg = np.mean(self.metrics_history[name][-5:])  # середнє за останні 5 епох
    ...
    def _update_best_metrics(self, metrics: Dict[str, float]):
        ...
        for name, value in metrics.items():
            ...
            self.writer.add_scalar(f'Best/{name}', self.best_metrics[name], self.global_step)
